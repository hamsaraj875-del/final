...
let chart = null;
let lastNotificationTimestamp = 0;     // 🔥 real time (ms)
const NOTIF_INTERVAL_MS = 5 * 60 * 1000; // 5 minutes EXACT

let currentNotification = null;
let lastNotificationTime = 0; // seconds
const NOTIF_INTERVAL = 300; // 5 minutes
...
function startMonitoring() {
  ...
  timerInterval = setInterval(() => {
    ...
    // 🔔 LIVE notification logic (every 5 mins after crossing limit)
    if (maxLimit > 0 && energyUsed > maxLimit) {
      if (
        !limitCrossedThisSession ||
        now - lastNotificationTimestamp >= NOTIF_INTERVAL_MS
      ) {
        limitCrossedThisSession = true;
        lastNotificationTimestamp = now;

        currentNotification =
          `⚠️ Energy limit crossed!\n` +
          `Used: ${energyUsed.toFixed(2)} Wh\n` +
          `Limit: ${maxLimit} Wh\n` +
          `Time: ${formatHMS(secondsPassed)}`;

        showNotificationsBox(); // 🚀 instant UI update
      }
    }
...
function stopMonitoring() {
  ...
  // ✅ Save history (needed for graph & suggestions)
  history.push({
...
function showSuggestionBox() {
  ...
  // Loop through each session in history
  history.forEach(session => {
    // Calculate total power of all appliances in this session
    const totalPower = session.appliances.reduce((sum, name) => {
      const app = appliances.find(a => a.name === name);
      return sum + (app ? app.power : 0);
    }, 0);

    // Assign energy proportional to each appliance's power
    session.appliances.forEach(name => {
      const app = appliances.find(a => a.name === name);
      if (app && totalPower > 0) {
        const appEnergy = session.energy * (app.power / totalPower);
        energyUsage[name] = (energyUsage[name] || 0) + appEnergy;
      }
    });
  });

  // Find the appliance with the maximum energy usage
  let maxApp = null;
  let maxEnergy = 0;
  for (const app in energyUsage) {
    if (energyUsage[app] > maxEnergy) {
      maxEnergy = energyUsage[app];
      maxApp = app;
    }
  }
...
function showNotificationsBox() {
  ...
  const div = document.createElement('div');
  div.className = 'notif';
  div.innerText = currentNotification;
  notificationsBox.appendChild(div);
}
